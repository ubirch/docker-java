...
function build_container() {
  ...
  if [ ! -f jdk-8u161-linux-x64.tar.gz ]; then
    ...
    curl -L "https://github.com/frekele/oracle-java/releases/download/8u161-b12/jdk-8u161-linux-x64.tar.gz" -O jdk-8u161-linux-x64.tar.gz
  fi
  mkdir -p VAR && docker build --build-arg JAVA_VERSION=${JAVA_VERSION:=8} \
    --build-arg JAVA_UPDATE=${JAVA_UPDATE:=161} \
    --build-arg JAVA_BUILD=${JAVA_BUILD:=-b12} \
    --build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` \
    --build-arg VCS_REF=`git rev-parse --short HEAD` \
    -t ubirch/java:v${GO_PIPELINE_LABEL} .
    if [ $? -eq 0 ]; then
      echo ${JAVA_VERSION:=8} > VAR/JAVA_VERSION
      echo ${JAVA_UPDATE:=161} > VAR/JAVA_UPDATE
      echo ${JAVA_BUILD:=-b12} > VAR/JAVA_BUILD
